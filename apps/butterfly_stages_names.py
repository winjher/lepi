...
def classify_images(image_path):
    ...
    try:
        ...
        predicted_class_name = stages_names[predicted_class_index]
        predicted_score = np.max(result) * 100

        # Correctly get the treatment information using the predicted index
        predicted_stages_info = stages_details[predicted_class_index]

        outcome = {
            "class_name": predicted_class_name,
            "score": predicted_score,
            "stages_info": predicted_stages_info, # This now holds the detailed treatment
        }
        return outcome
    except Exception as e:
...
uploaded_file = st.file_uploader('Upload an Image of a Butterfly Stages')
if uploaded_file is not None:
    try:
        ...
        if classification_result:
            st.markdown(
                f"**Predicted Stages:** **{classification_result['class_name']}** "
                f"with a confidence score of **{classification_result['score']:.2f}%**."
            )
            st.markdown(f"**StagesInformation:** {classification_result['stages_info']}")
            ...
            stages_data = {
                "stages_name": [classification_result['class_name']], # Use 'class_name'
                "stages_info": [classification_result['stages_info']],
                "score": [classification_result['score']],
                "date_time": [date_time],
                "location": [location],
                "image_path": [uploaded_file.name],  # Store filename
            }
            df_species = pd.DataFrame(stages_data)

            # Append to CSV (or create if it doesn't exist)
            csv_file = "./data/stages_records.csv" # Renamed for clarity
            data_dir = './data'
